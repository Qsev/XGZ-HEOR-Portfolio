...
## Executive Summary: Bridging Data Asymmetry in HTA {#1-executive-summary}
...
In this chapter, we demonstrate the mathematical mechanism of weighting using the simpler Unanchored scenario to focus purely on the weighting mechanics.
-->
```
...
### Entropy Balancing vs. Inverse Probability Weighting (IPW)
...
### Effective Sample Size (ESS) Diagnosis
...
## Sensitivity Analysis in MAIC {#7-sensitivity}
...
```{=html}
<!-- Audit Point: Covariate Selection and the Bias-Variance Trade-off
In our simple mock case, we only have one covariate (Age), so there is no choice to make. However, in real-world HTA submissions, the competitor's publication might report a dozen baseline characteristics. 
Modellers face a severe "Bias-Variance Trade-off":
- Matching on all available covariates guarantees perfect balance but often causes the ESS to plummet to an unacceptably low number (as extreme weights are assigned to a few patients to achieve balance in a high-dimensional space).
- Matching on too few covariates preserves a large ESS but risks leaving residual confounding.
This is why presenting multiple "Covariate Selection Scenarios" is a mandatory part of any MAIC sensitivity analysis requested by agencies like NICE.
-->
```
