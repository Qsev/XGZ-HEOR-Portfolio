...
## Executive Summary: The Anatomy of a Network {#1-executive-summary-the-anatomy-of-a-network}
...
## Theoretical Framework: The Calculus of Consistency {#2-theoretical-framework-the-calculus-of-consistency}
...
### The Likelihood (Binomial-Logit Model)
...
### The Consistency Assumption (The Core Constraint)
...
::: callout-important
### Judgment Logic for FE vs. RE (HTA Audit Focus)
...
3. **Model Fit (DIC)**: Compare the Deviance Information Criterion (DIC) between FE and RE models, alongside the residual deviance. A smaller DIC indicates a better fit, and the DIC already penalises the extra parameter the RE model brings. NICE DSU TSD 2 recommends consulting both and sets no numerical threshold: the difference of 3-5 points often quoted is a rule of thumb from the wider DIC literature, not from TSD 2. When the two models are close, the choice rests on the data-generating process and the interpretability of the results. 
...
## The Minimal Mock Case: A vs B vs C {#3-the-minimal-mock-case-a-vs-b-vs-c}
...
## MCMC Forensic Audit: How Bayes Actually Solves the Network {#4-mcmc-forensic-audit-implementation--convergence}
...
### Step 1: Setting up the Likelihood and Parameters
...
### Step 2: Assigning Priors (The Bayesian Part)
...
::: callout-tip
### Prior Choice in Real-World NMA
...
### The Iterative Algorithm and Posterier.
...
### Model Implementation: BUGS/JAGS & Stan
...
# Prepare data (same as the Mock Data table)
stan_data <- list(
    J = 3,
    r = matrix(c(20, 40, 20, 50, 30, 40), nrow = 3, byrow = TRUE),
    n = matrix(c(100, 100, 100, 100, 100, 100), nrow = 3, byrow = TRUE)
...
# Summarise the posterior
pars <- c("d_AB", "d_AC", "OR_AB", "OR_AC", "OR_BC")
stan_sum <- summary(fit, pars = pars)$summary
stan_tbl <- stan_sum[, c("mean", "sd", "2.5%", "50%", "97.5%", "n_eff", "Rhat")]
kable(stan_tbl, digits = c(3, 3, 3, 3, 3, 0, 3), col.names = c("Mean", "SD", "2.5%", "Median", "97.5%", "ESS", "R-hat"))
```
...
## Results & Interpretation: The League Table {#5-results--interpretation-the-league-table}

After running the MCMC, we combine the direct and indirect evidence to get the final pooled estimates. Everything below is computed from the posterior draws of the Stan fit above.

### The League Table (Median OR and 95% CrI)

```{r league-table}
library(knitr) # the chunks above may be served from cache, so load it here too

post <- rstan::extract(fit, pars = c("d_AB", "d_AC"))
d <- cbind(A = 0, B = post$d_AB, C = post$d_AC) # log OR of each treatment vs A, one row per draw
trts <- colnames(d)
...
for (i in 1:3) {
    for (j in 1:3) {
        if (i == j) {
            league[i, j] <- paste0("<strong>Treatment ", trts[i], "</strong>")
        } else {
            or_draws <- exp(d[, j] - d[, i]) # OR of column treatment vs row treatment
            q <- quantile(or_draws, c(0.5, 0.025, 0.975))
            league[i, j] <- sprintf("%.2f (%.2f, %.2f)", q[1], q[2], q[3])
        }
    }
}
kable(league, format = "html", col.names = NULL, align = "c", escape = FALSE, table.attr = 'class="table table-sm"')

# Probability of being best: share of draws in which each treatment has the highest log OR
p_best <- table(factor(trts[apply(d, 1, which.max)], levels = trts)) / nrow(d)
...
Bayesian NMA allows us to calculate the probability that each treatment is the most effective: in each posterior draw, the treatment with the highest log odds of response is counted as best.

- **Treatment C**: `r pct(p_best["C"])`
- **Treatment B**: `r pct(p_best["B"])`
- **Treatment A**: `r pct(p_best["A"])`

## Strategic Q&A: Interview Prep {#6-strategic-qa-interview-prep}
...
## Strategic Conclusion {#7-strategic-conclusion}
