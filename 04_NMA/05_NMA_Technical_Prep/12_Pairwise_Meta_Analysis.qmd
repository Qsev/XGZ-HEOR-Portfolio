...
kable(
    rbind(pars_fe, pars_re)[, c("mean", "sd", "2.5%", "50%", "97.5%", "n_eff", "Rhat")],
...
The first two rows are the fixed-effect model, the rest the random-effects model. Neither sampler reported any divergent transitions (`r n_div(bayes_fe)` and `r n_div(bayes_re)`), the largest R-hat is `r sprintf("%.3f", max(c(pars_fe[, "Rhat"], pars_re[, "Rhat"])))`, and the smallest effective sample size is `r format(round(min(c(pars_fe[, "n_eff"], pars_re[, "n_eff"]))), big.mark = ",")`.
