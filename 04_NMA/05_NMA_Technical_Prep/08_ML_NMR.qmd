---
title: "08. Multilevel Network Meta-Regression (ML-NMR)"
...
$$ \text{logit}(P_{ik}) = \ln (\dfrac{P_{ik}}{1-P_{ik}}) = \mu_k + \beta_{age}\text{Age}_i + \beta_{trt_k}\text{Trt}_i + \beta_{age:trt}(\text{Age}_i \times \text{Trt}_i) $$
...
The specific formulas are derived from the linear predictor equation:
$$\text{logit}(p) = \mu + \beta_{age} \cdot (\text{Age} - c) + d_{trt} \cdot \text{trt} + \beta_{age:trt} \cdot (\text{Age} - c) \cdot \text{trt}$$

To find the parameters at $\text{Age} = 45$:

1. **Adjusted Intercept ($\mu'$)**:
   $$\mu'_{k} = \mu_k + \beta_{age} \cdot (45 - c)$$
   *The intercept shifts by the age effect over the distance of ~10.08 years.*

2. **Adjusted Treatment Effect ($d'_{trt}$)**:
   $$d'_{trt} = d_{trt} + \beta_{age:trt} \cdot (45 - c)$$
   *The main treatment effect shifts by the interaction effect over the same distance.*

3. **Slopes ($\beta_{age}, \beta_{age:trt}$)**:
   These are rates of change per year and are invariant to the centering point. Therefore, no adjustment is needed.
...
2. **Calculate the Linear Predictor ($\eta$)**:
...
3. **Convert to Probability (Inverse Logit)**:
   $$P = \frac{1}{1 + e^{-`r round(eta_35, 2)`}} \approx \mathbf{`r round(p_35 * 100, 1)`\%}$$
...
For the Quizartinib vs Midostaurin comparison:
$$\text{OR} = e^{-0.664} \approx 0.51$$
