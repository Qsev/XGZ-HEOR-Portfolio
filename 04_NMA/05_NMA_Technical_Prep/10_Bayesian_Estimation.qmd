...
**The interval widens**, by about 40%, and by the same amount in both draws. The reason is how evidence accumulates. The log-likelihood is a sum with one term per observation, so every observation adds the same amount of curvature around the peak. With 15 terms instead of 30 the peak is flatter: a wider range of $\theta$ values explains the data almost equally well, and the posterior spreads out accordingly.
