...
We fit a logistic regression model[^glm_link] for $Y$ and generate two counterfactual predicted values for each individual $i$:
...
[^glm_link]: Specifying `family = binomial` in R's `glm()` function defaults to a logistic regression model (Logit link function) to model binary outcome probabilities.
