...
Specifically, the E-value is defined as the minimum strength of association on the risk ratio scale that an unmeasured confounder ($U$) must have with both:
1. The treatment assignment ($D$)
2. The clinical outcome ($Y$)

to fully explain away the observed treatment-outcome association, adjusting for the measured covariates ($W$).

#### How is it Calculated?

For an observed Relative Risk ($RR$) greater than $1$ (indicating a risk-increasing treatment effect, or when we invert a protective effect $RR < 1$ to $1/RR$):
