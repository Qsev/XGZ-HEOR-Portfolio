...
### Cohort Simulation Setup

First, we set up the simulation environment and generate a baseline cohort where elderly and sicker patients (with higher ECOG performance status) are preferentially assigned to treatment (confounding by indication).
...
For $N=500$, the lower bound is approximately `0.03598`. We fit a logistic regression for treatment and apply this bound independently to $g(W)$ (treated) and $1 - g(W)$ (control) to guarantee that inverse weights do not blow up.
