...
A Partitioned Survival Model (PartSM) is defined by two independent survival functions — PFS and OS — that jointly determine the proportion of the cohort in each of three mutually exclusive health states at every model cycle:
...
A non-negotiable biological constraint is that OS cannot fall below PFS at any cycle:

$$S_{\text{OS}}(t) \geq S_{\text{PFS}}(t) \quad \forall\, t$$

Violations of this constraint — which can arise from naive extrapolation — must be corrected programmatically. Any model submitted to NICE without this safeguard will be immediately flagged by the EAG.
...
$$\text{QALY} = \sum_{t=0}^{T} \left[\pi_{\text{PFS}}(t) \cdot u_{\text{PFS}} + \pi_{\text{PD}}(t) \cdot u_{\text{PD}}\right] \cdot \frac{1}{12} \cdot (1 + r)^{-t/12}$$
