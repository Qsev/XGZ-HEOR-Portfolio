...
get_mcm_row <- function(fit, cohort_name, endpoint, dist_name) {
  if (inherits(fit, "flexsurvcure")) {
    ...
    data.frame(
      ...
      `Parameter 1` = sprintf("%s = %.4f", p1_name, p1_val),
      `Parameter 2` = sprintf("%s = %.4f", p2_name, p2_val),
      check.names = FALSE
    )
  } else {
...
knitr::kable(
  ...
  col.names = c("Cohort", "Endpoint", "Model Type", "Distribution", "Cure Fraction (\u03b8)", "Parameter 1", "Parameter 2")
)
```
...
$$\mathbf{Trace}_{\text{CAR-T}}(t) = P_1 \cdot \mathbf{Trace}_{\text{CAR-T, infused}}(t) + P_2 \cdot (0.5 \cdot \mathbf{Trace}_{\text{Blina}}(t) + 0.5 \cdot \mathbf{Trace}_{\text{Chemo}}(t)) + P_3 \cdot [0, 0, 1]^T$$
...
1. **Base Treatment Disutility (Hospitalization)**: A decrement of **-0.42** (based on Sung et al., 2003) is applied for the duration of treatment hospitalization:
   ...
   - **Blinatumomab**: 21.00 days (EAG swapped value; Document B original is 9.24 days)
   - **FLAG-IDA**: 9.24 days (EAG swapped value; Document B original is 21.00 days)
   
   *Note: In accordance with the user instructions, we apply the 21.00 days duration for Blinatumomab and 9.24 days for FLAG-IDA. An auditing note on this parameter swap is documented in the technical portfolio.*

2. **Severe CRS (ICU Stay)**: A decrement of **-0.91** is applied for a duration of 11.10 days:
   - **Tisagenlecleucel**: incidence of **48.10%**
   - **Blinatumomab**: incidence of **5.71%**
...
4. **Subsequent Stem Cell Transplant (allo-SCT)**: A decrement of **-0.57** is applied for a duration of 365 days, weighted by the proportion of patients receiving subsequent transplant:
   - **Tisagenlecleucel**: **22.78%**
   - **Blinatumomab**: **34.29%**
   - **FLAG-IDA**: **14.75%**

### Age-Adjustment and Discounting

#### Age-Related Utility Multipliers
...
# Durations (days)
dur_tisa_tx  <- 25.85
dur_blin_tx  <- 21.00  # EAG swapped value
dur_chemo_tx <- 9.24   # EAG swapped value
dur_crs_icu  <- 11.10
dur_non_crs  <- 1.74
dur_sct      <- 365.00
...
get_age_multiplier <- function(age) {
  case_when(
    ...
    age <= 73 ~ 0.85,
    age <= 75 ~ 0.84,
    TRUE      ~ pmax(0.84 - floor((age - 75) / 2) * 0.01, 0.50)
  )
}
...
- **Tisagenlecleucel List Price**: £282,000.00
- **NHS CAR-T Service Tariff**: £41,101.00 (covers leukapheresis, cell infusion, and early monitoring/AEs up to 100 days)
- **Pre-treatment / Pre-infusion Costs**:
  - Bridging chemotherapy: £1,394.57
  - Lymphodepleting chemotherapy: £404.52
  - Leukapheresis (only for uninfused patients): £2,575.70
- **Tisagenlecleucel Long-term AE Monitoring (B-cell aplasia / IVIg)**: £11,176.85
- **Subsequent Allogeneic Stem Cell Transplant (allo-SCT)**: £151,227.43 (applied to transplant rates: Tisa = 22.78%, Blina = 34.29%, Chemotherapy = 14.75%)
- **Comparator Total Treatment Costs (drug + admin + hospitalization)**:
  - Blinatumomab: £89,436.21
  - Salvage Chemotherapy (FLAG-IDA): £21,660.00
- **Comparator Short-Term AE Management**: Blina = £2,847.49; Chemotherapy = £1,802.56

#### Monthly Health State Follow-up Costs

| Cohort | Health State | Months 1–12 | Months 13–24 | Months 25–60 | Months >60 |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
##### Original List for Verification
- **Tisagenlecleucel (EFS state)**: Months 1-12 = £472.58; Months 13-24 = £111.87; Months 25-60 = £56.18; >60 months = £27.47
- **Comparators (EFS state)**: Months 1-12 = £263.00; Months 13-24 = £110.86; Months 25-60 = £55.43; >60 months = £27.47
- **Tisagenlecleucel (PD state)**: Months 1-12 = £191.00; Months 13-60 = £263.00; >60 months = £27.47
- **Comparators (PD state)**: Months 1-60 = £263.00; >60 months = £27.47
- **Terminal Care Cost**: £13,198.42 (applied to deaths occurring within the first 5 years; for infused tisagenlecleucel, early deaths within 100 days post-infusion are covered by the NHS service tariff and therefore excluded).
