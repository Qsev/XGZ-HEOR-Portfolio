...
This section loads analytical packages, establishes working directories, and imports the patient-level survival data (IPD) reconstructed from digitized KEYNOTE-868 (NRG-GY018) trial curves (PFS and OS for dMMR and pMMR cohorts) using the Guyot et al. iterative algorithm.
...
# Reconstructed IPD sample sizes (Total N = `r nrow(ipd)`)
ipd |>
  count(subgroup, endpoint, treatment, status_label) |>
  arrange(subgroup, endpoint, treatment, status_label) |>
  kable(col.names = c("Subgroup", "Endpoint", "Treatment Arm", "Event Status", "Sample Size (N)"))
```
