...
Plotting the curves provides a clearer view of their long-term shapes. The plots below show the extrapolations for two critical decision curves: **pMMR PFS** (which dictates progression-free occupancy in the PSM model) and **dMMR OS** (which exhibits high immaturity).
...
Before assessing the tail, we check how well the parametric fits align with the observed trial period. The plot below overlays the reconstructed KM step curve with the AIC-best fit for **pMMR PFS**.
...
### Step 2: EAG Issue 3 Audit (Trial Data Immaturity)

#### The Core Appraisal Critique

The EAG's Issue 3 points out that the KEYNOTE-868 trial has a short follow-up (median of approximately **16.3 months**), whereas the model projects survival outcomes over a multi-decade horizon. This means the majority of the projected survival area (and thus QALY gains) is model-driven rather than observed.

The tables below translate the EAG's data immaturity concerns into concrete modeling issues and maturity checks.
...
In HTA auditing, we evaluate model maturity across four standard dimensions:
...
To demonstrate the structural uncertainty of this extrapolation, we plot all seven candidate distributions on the same curve. While they all fit the observed trial period similarly, they diverge massively in the extrapolation zone.
...
#### Landmark Survival Comparisons

This divergence is quantified numerically below, showing the survival estimates projected by each candidate distribution at key clinical landmarks (2, 3, 5, 10, and 20 years).
...
#### Lead Modeler Appraisal Synthesis

The landmark predictions reveal substantial structural uncertainty. For instance, at 20 years (240 months), the log-normal model projects a **30.3%** survival rate, whereas the exponential model projects **0.3%**. 

This highlights why choosing a curve based on statistical fit (AIC/BIC) alone is insufficient. Both log-normal and log-logistic models have very similar statistical fits during the observed trial period, yet they lead to completely different lifetime clinical and economic outcomes. 

In HTA submissions, modelers must justify their choice of extrapolation curve not just on statistical diagnostics, but on **clinical tail plausibility** (e.g., matching projected long-term survival to real-world cancer registries, registry-based mortality statistics, or clinical expert consensus on cure rates).
